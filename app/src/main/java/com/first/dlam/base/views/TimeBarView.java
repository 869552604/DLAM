...
public class TimeBarView extends View {
    ...
    private float barInterval = dp2px(1);//bar之间的距离
    private int mContentPaintColor = Color.parseColor("#000000"); //最左边文字的颜色
    private int mbarPaintColor = Color.parseColor("#0EBF93");//bar的颜色
    ...
    private float topAndBottomInterval = dp2px(1);//上下边距的距离（上下线与bar的距离）
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < hoBarEntityList.size(); i++) {
            ...
            //绘制bar
            if(hoBarEntityList.get(i).type==0){
                mbarPaint.setColor(Color.parseColor("#0EBF93"));
            }else if(hoBarEntityList.get(i).type==1){
                mbarPaint.setColor(Color.parseColor("#FF0000"));
            }else{
                mbarPaint.setColor(Color.parseColor("#FF7D00"));
            }
            ...
            if(hoBarEntityList.get(i).type==2){
                canvas.drawText(hoBarEntityList.get(i).info, barEndX + viewIntervar * 2, countBaseLine, mCountPaint);
            }else {
                canvas.drawText(hoBarEntityList.get(i).count + "手", barEndX + viewIntervar * 2, countBaseLine, mCountPaint);
            }
...
//        maxCount=0;
    ...
    public static class HoBarEntity {
        ...
        public HoBarEntity(String content,int count,int type,String info){
