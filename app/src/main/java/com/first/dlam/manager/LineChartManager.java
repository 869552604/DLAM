package com.first.dlam.manager;

import android.content.Context;
import android.graphics.Color;
import android.graphics.drawable.Drawable;

import com.first.dlam.base.views.LineChartMarkView;
...
public class LineChartManager {
    ...
    private float Y; //设置Y轴起步值。
    private int YSize;
    private LineData lineData;
    private LineDataSet lineDataSet;
    private List<ILineDataSet> lineDataSets = new ArrayList<>();

    public LineChartManager(LineChart lineChart, float Y, int YSize, List<String> names, List<Integer> colors) {
    ...
    public LineChartManager(LineChart lineChart, float Y, int YSize) {
        this.lineChart = lineChart;
        leftYAxis = lineChart.getAxisLeft();
        rightYAxis = lineChart.getAxisRight();
        xAxis = lineChart.getXAxis();
...
//        lineChart.animateX(500);
        ...
        xAxis.setDrawGridLines(true); //X轴是否显示网络格
        rightYAxis.setDrawGridLines(false);
        leftYAxis.setDrawGridLines(true);//左边Y轴是否显示网络格
        //设置X轴网格线为虚线
        ...
        //保证Y轴从0开始，不然会上移一点
        leftYAxis.setAxisMinimum(Y);
        rightYAxis.setAxisMinimum(Y);
    ...
    private void initLineDataSet(LineDataSet lineDataSet, int color, LineDataSet.Mode mode) {
        ...
        lineDataSet.setDrawCircles(false);//设置是否显示原点
        lineDataSet.setDrawValues(true); //设置是否显示顶点的值
        //设置曲线值的圆点是实心还是空心
...
//        xAxis.setDrawScale(false);
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                return String.valueOf((int)value); //显示X轴下方坐标
            }
        });

        leftYAxis.setLabelCount(YSize);
//        leftYAxis.setDrawScale(true);
        ...
        leftYAxis.setValueFormatter(new IAxisValueFormatter() {//显示左边Y轴的值
            @Override
            public String getFormattedValue(float value, AxisBase axis) {
                return getData((double) value);
            }
        });
        ...
        //线条自定义内容 放在这里
        lineDataSet.setValueFormatter(new IValueFormatter() {//显示每个顶点的值
            @Override
    ...
    public void showLineChart2(final List<IncomeBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            IncomeBean data = dataList.get(i);
            /**
...
//        leftYAxis.setDrawScale(true);
    ...
    public void showLineChart3(final List<IncomeBean> dataList, String name, int color) {
        List<Entry> entries = new ArrayList<>();
        for (int i = 0; i < dataList.size(); i++) {
            IncomeBean data = dataList.get(i);
            /**
